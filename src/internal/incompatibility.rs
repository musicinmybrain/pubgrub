...
impl<P: Package, VS: VersionSet> Incompatibility<P, VS> {
    ...
    pub fn from_dependency(package: Id<P>, version: VS, dep: (Id<P>, &VS)) -> Self {
        let (p2, set2) = dep;
        Self {
            package_terms: if set2 == &VS::empty() {
                SmallMap::One([(package, Term::Positive(version.clone()))])
            } else {
                SmallMap::Two([
                    (package, Term::Positive(version.clone())),
                    (p2, Term::Negative(set2.clone())),
                ])
            },
            kind: Kind::FromDependencyOf(package, version, p2, set2.clone()),
        }
    }
    ...
    pub fn build_derivation_tree(
        ...
        match store[self_id].kind.clone() {
            ...
            Kind::UnavailableDependencies(package, set) => DerivationTree::External(
                External::UnavailableDependencies(package_store[package].clone(), set),
            ),
