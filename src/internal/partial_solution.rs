...
type SatisfiedMap<P, VS> = SmallMap<Id<P>, (Option<IncompId<P, VS>>, u32, DecisionLevel)>;
...
impl<P: Package, VS: VersionSet, Priority: Ord + Clone> PartialSolution<P, VS, Priority> {
    ...
    pub fn extract_solution(&self) -> impl Iterator<Item = (Id<P>, VS::V)> + '_ {
        self.package_assignments
            .iter()
    ...
    fn find_previous_satisfier(
        ...
        mut satisfied_map: SatisfiedMap<P, VS>,
        package_assignments: &FnvIndexMap<Id<P>, PackageAssignments<P, VS>>,
        store: &Arena<Incompatibility<P, VS>>,
    ) -> DecisionLevel {
